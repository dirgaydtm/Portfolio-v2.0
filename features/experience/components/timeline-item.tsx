...
export default function TimelineItem({ item, isRight, className }: TimelineItemProps) {
    ...
    return (
        <div className={`relative flex items-start pb-12 last:pb-0 ${isRight ? "md:justify-end" : "md:justify-start"} ${className}`}>
            ...
            <div className={`ml-16 md:ml-0 md:w-[calc(50%-2rem)] space-y-3 text-left ${alignClass}`}>
                <div className={`flex items-center gap-2.5 h-9 ${isRight ? "md:flex-row" : "md:flex-row-reverse"}`}>
                    <Building2 className="size-5  shrink-0 h-full hidden md:flex text-muted-foreground" />
                    <span className="text-base font-medium text-foreground">{item.company}</span>
                </div>
                <div>
                    <h3 className="text-xl font-semibold tracking-tight text-foreground">{item.title}</h3>
                    <div className={`flex items-center gap-2 mt-1 text-sm text-muted-foreground ${justifyClass}`}>
